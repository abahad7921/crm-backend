...
router.post("/", newUserValidation, async (req, res) => {
	...
	try {
		...
		let url = await emailProcessor({
			email,
			type: "new-user-confirmation-required",
			verificationLink: verificationURL + result._id + "/" + email,
		});

		res.json({ status: "success", message: "New user created", result, previewUrl: url });
	} catch (error) {
		console.log(error);
...
router.patch("/reset-password", updatePassValidation, async (req, res) => {
	...
	if (getPin?._id) {
		...
		if (user._id) {
			...
			let url = await emailProcessor({ email, type: "update-password-success" });
